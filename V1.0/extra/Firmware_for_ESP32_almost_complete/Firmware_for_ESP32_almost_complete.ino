...
byte read_card_rec_from_arduino[4];
...
void get_input_from_arduino_uno(){
  String data_from_arduino_uno;
  bool stop_rec_data = false;
  int i = 0;
  while (stop_rec_data == false){
      bus.tick();
      if (bus.gotData()) {
        myStruct data;
        bus.readData(data);
        read_card_rec_from_arduino[i] = data.x;
        i++;
        if (i > 3)
          stop_rec_data = true;
      }
...
void get_input_from_arduino_uno_lock_scr(){
  String data_from_arduino_uno;
  bool stop_rec_data = false;
  int i = 0;
  while (stop_rec_data == false){
...
void lock_scr_with_rfid() {
  ...
  get_input_from_arduino_uno_lock_scr();
  for (int i = 0; i < 4; i++){
    read_cards[i + 4] = read_card_rec_from_arduino[i];
  }
  ...
  get_input_from_arduino_uno_lock_scr();
  for (int i = 0; i < 4; i++){
    read_cards[i + 8] = read_card_rec_from_arduino[i];
  }
  new_screen = chosen_lock_screen;
  while (chosen_lock_screen == new_screen) {
    new_screen = esp_random() % 9;
  ...
  get_input_from_arduino_uno_lock_scr();
  for (int i = 0; i < 4; i++){
    read_cards[i + 12] = read_card_rec_from_arduino[i];
  }
...
void approximate_operator_card_to_continue(byte cps) {
  ...
  for (int i = 0; i < 4; i++){
    if (read_card_rec_from_arduino[i] < 16)
      read_card += "0";
     read_card += String(read_card_rec_from_arduino, HEX);
  }
